/* $Id: window.cpp 18620 2009-12-23 22:41:47Z rubidium $ */
...
#define scrollspeed 1
			if (x - 35 < 0) {
				w->viewport->dest_scrollpos_x += ScaleByZoom((x - 35) * scrollspeed, vp->zoom);
			} else if (35 - (vp->width - x) > 0) {
				w->viewport->dest_scrollpos_x += ScaleByZoom((35 - (vp->width - x)) * scrollspeed, vp->zoom);
			}
			if (y - 35 < 0) {
				w->viewport->dest_scrollpos_y += ScaleByZoom((y - 35) * scrollspeed, vp->zoom);
			} else if (35 - (vp->height - y) > 0) {
				w->viewport->dest_scrollpos_y += ScaleByZoom((35 - (vp->height - y)) * scrollspeed, vp->zoom);
			}
#undef scrollspeed
		}
	}
