/* $Id: sdl_v.cpp 18547 2009-12-19 19:29:01Z rubidium $ */
...
static bool CreateMainSurface(uint w, uint h)
{
...
#if defined(__APPLE__) && defined(__arm__)
	newscreen = SDL_CALL SDL_SetVideoMode(480, 320, 32, SDL_SWSURFACE | SDL_HWPALETTE | SDL_FULLSCREEN | SDL_NOFRAME);
#else
  ...
  memset(_screen.dst_ptr, 0, 480*320*4);
	fprintf(stderr, "SDL: set mode %ux%ux%d 0x%x\n", newscreen->w, newscreen->h, _screen.pitch, _screen.dst_ptr);
...
void VideoDriver_SDL::MainLoop()
{
  int old_x = 0, old_y = 0, count_xy = 0;
	...
	for (;;) {
      ...
      /*
      int x, y;
      if(SDL_GetMouseState(0, &x, &y) & SDL_BUTTON(SDL_BUTTON_LEFT) && SDL_GetMouseState(1, &x, &y) & SDL_BUTTON(SDL_BUTTON_LEFT))
      {
        if(count_xy > 1)
        {
          if(x < old_x - 15)
